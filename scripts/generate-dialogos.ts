...
function validar(n: number, d: DialogosDoCircuito, blocos: string[]) {
  ...
  for (const [nome, falas] of [
    ...
    for (const [quem, en, pt] of falas) {
      ...
      const semContracao = en.match(SEM_CONTRACAO);
      exigir(!semContracao, `${nome}: "${en}" usa "${semContracao?.[0]}" — o curso exige contração`);
    }
  ...
  const minimo = Math.max(2, Math.round(blocos.length * 0.25));
  exigir(
...
async function main() {
  ...
  const blocosJson = JSON.parse(
    readFileSync(join(process.cwd(), "content", "metodo", "blocos.json"), "utf8"),
  ) as { n: number; blocos: { en: string; pt: string }[] }[];
  const blocosPorN = new Map(blocosJson.map((c) => [c.n, c.blocos]));

  const todos = load();
  const prontos = new Set(todos.map((c) => c.n));

  let alvos = RAMPA.map((c) => c.n).filter((n) => blocosPorN.has(n));
    ...
    for (let t = 1; t <= TENTATIVAS && !feito; t++) {
      try {
        ...
        const montado: DialogosDoCircuito = {
          n,
          elenco: vindo.elenco ?? [],
          imersao: (vindo.imersao ?? []).map((f) => [f.quem, f.en, f.pt] as Fala),
          escuta: (vindo.escuta ?? []).map((f) => [f.quem, f.en, f.pt] as Fala),
          deriva: vindo.deriva ?? [],
        };
