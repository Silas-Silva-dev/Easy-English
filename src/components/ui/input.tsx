...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "border-input bg-card flex h-10 max-sm:h-11 w-full min-w-0 rounded-lg border px-3 py-2 text-sm max-sm:text-base shadow-xs transition-[color,box-shadow,border-color] outline-none",
...
function Select({ className, children, ...props }: React.ComponentProps<"select">) {
  return (
    <select
      data-slot="select"
      className={cn(
        "border-input bg-card h-10 max-sm:h-11 w-full rounded-lg border px-3 text-sm max-sm:text-base shadow-xs transition-[color,box-shadow,border-color] outline-none",
        "focus-visible:border-ring focus-visible:ring-ring/25 focus-visible:ring-[3px]",
        "disabled:cursor-not-allowed disabled:opacity-60",
        className,
      )}
